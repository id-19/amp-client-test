...
import { getAccountBalancesDomain, getAccountDomain, getSignerDomain, getGetStartedModalDomain, } from '../domains';
...
export const convertETH = (convertAmount: number): ThunkAction => {
  return async (dispatch, getState, { mixpanel }) => {
    mixpanel.track('convert-eth', {}); // Updated tracking event here with properties
    try {
      ...
      convertTxReceipt.status === 0 ? dispatch(actionCreators.revertConvertTx(convertTxReceipt)) : dispatch(actionCreators.confirmConvertTx(convertTxReceipt));
      allowTxReceipt.status === 0 ? dispatch(actionCreators.revertApproveTx(allowTxReceipt)) : dispatch(actionCreators.confirmApproveTx(allowTxReceipt));
      (convertTxReceipt.status === 0 || allowTxReceipt.status === 0) ? dispatch(notificationActionCreators.addErrorNotification({ message: 'ETH conversion transaction failed' })) : dispatch(notificationActionCreators.addSuccessNotification({ message: 'ETH conversion transaction successful!' }))
    } catch (error) {
...
export const approveWETH = (): ThunkAction => {
  return async (dispatch, getState, { mixpanel }) => {
    mixpanel.track('approve-weth', {}); // Updated tracking event here with properties
    try {
      let signer = getSigner()
      let networkID = signer.networkID
      let weth = new Contract(WETH_ADDRESS[networkID], WETH, signer)
      ...
      txReceipt.status === 0 ? dispatch(actionCreators.revertApproveTx(txReceipt)) : dispatch(actionCreators.confirmConvertTx(txReceipt))
    } catch (error) {
      console.log(error.message)
    }
  }
