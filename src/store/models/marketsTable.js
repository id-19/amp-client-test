...
import { getAccountDomain, getTokenPairsDomain, getAccountBalancesDomain, getTokenDomain } from '../domains'
...
export default function marketsTableSelector(state: State) {
  ...
  return { pairs, quoteTokens, currentReferenceCurrency: referenceCurrency.symbol }
}

export function redirectToTradingPage(pairSymbol: string): ThunkAction {
  return async (dispatch, getState, { mixpanel }) => {
    mixpanel.track('market-page/redirect-to-trading-page', { action: "Navigation to trading page" })
  ...
  return async (dispatch, getState, { txProvider }) => {
    try {
      ...
      const approvalConfirmedHandler = (txConfirmed) => {
        txConfirmed ? dispatch(notifierActionCreators.addSuccessNotification({ message: `${symbol} Approval Successful. You can now start trading!` })) : dispatch(notifierActionCreators.addErrorNotification({ message: `${symbol} Approval Failed. Please try again.` }))
      }
      const approvalRemovedHandler = (txConfirmed) => {
        txConfirmed ? dispatch(notifierActionCreators.addSuccessNotification({ message: `${symbol} Allowance Removal Successful.` })) : dispatch(notifierActionCreators.addErrorNotification({ message: `${symbol} Allowance Removal Failed. Please try again.` }))
      }
      if (isAllowed) {
