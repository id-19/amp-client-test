...
import { getTokenPairsDomain, getAccountBalancesDomain, getOrdersDomain, getAccountDomain } from '../domains'
import * as actionCreators from '../actions/tokenSearcher'
import { getQuoteToken, getBaseToken } from '../../utils/tokens'
import { parseWETHPair, parseToWETHPair } from '../../utils/helpers'
...
export default function tokenSearcherSelector(state: State) {
    ...
    for (let quote of quotes) {
        ...
        tokenPairsByQuoteToken[quote] = tokenPairs
            ...
            .map(tokenPair => {
                ...
                return { ...tokenPair, pair: parsedPair, base: getBaseToken(parsedPair), quote: getQuoteToken(parsedPair) }
            })
            .map(tokenPair => ({ ...tokenPair, favorited: favoriteTokenPairs.indexOf(tokenPair.pair) > -1 }))
    }
    ...
    return { tokenPairsByQuoteToken, currentPair, baseTokenBalance, quoteTokenBalance, baseTokenAvailableBalance, quoteTokenAvailableBalance }
}
...
export const updateCurrentPair = (symbol: string): ThunkAction => {
    return async (dispatch, getState, { socket, mixpanel }) => {
        ...
        mixpanel.track('trading-page/update-current-pair', {
            "New Pair Symbol": newPairSymbol,
            "Action": "Update Current Trading Pair"
        });
        try {
