...
import { getAccountBalancesDomain, getAccountDomain, getDepositFormDomain, getSignerDomain, getTokenDomain, } from '../domains';
...
export function queryBalances(): ThunkAction {
  return async (dispatch, getState, { provider }) => {
    mixpanel.track('Query Balances', { Action: "Query", Scope: "All Balances" });
    try {
...
export function subscribeBalance(token: Token): ThunkAction {
  return async (dispatch, getState, { mixpanel, provider }) => {
    mixpanel.track('Subscribe Balance', { Action: "Subscribe", Token: token.symbol });
    try {
      let unsubscribe;
      const { symbol } = token;
      const state = getState();
      ...
      token.address === '0x0' ? (unsubscribe = await provider.subscribeEtherBalance(accountAddress, updateBalanceHandler)) : (unsubscribe = await provider.subscribeTokenBalance(accountAddress, token, updateBalanceHandler));
      return async () => {
        unsubscribe();
...
export const confirmEtherDeposit = (
  ...
  return async (dispatch, getState, { mixpanel }) => {
    mixpanel.track('Confirm Ether Deposit', { Action: "Deposit", Currency: "ETH", Amount: convertAmount });
    try {
      ...
      if (shouldConvert) {
        if (shouldAllow) {
          ...
          convertTxReceipt.status === 0 ? dispatch(depositFormActionCreators.revertConvertTx(convertTxReceipt)) : dispatch(depositFormActionCreators.confirmConvertTx(convertTxReceipt));
          allowTxReceipt.status === 0 ? dispatch(depositFormActionCreators.revertAllowTx(allowTxReceipt)) : dispatch(depositFormActionCreators.confirmAllowTx(allowTxReceipt));
        } else {
...
export const confirmTokenDeposit = (
  ...
  return async (dispatch, getState, { mixpanel }) => {
    mixpanel.track('Confirm Token Deposit', { Action: "Deposit", Token: address, Amount: "Dynamic based on user input" });
    try {
      let signer = getSigner();
