...
import { getTokenDomain, getAccountBalancesDomain, getAccountDomain, getTransactionsDomain } from '../domains'
...
export function detectContract(tokenAddress: string): ThunkAction {
  return async (dispatch, getState, { provider, api, mixpanel }) => {
    ...
    try {
      ...
      if (token) {
        return { isRegistered: true, decimals: token.decimals, symbol: token.symbol }
      }
      token = await provider.detectContract(tokenAddress)
      if (token && token.symbol) {
        return { isRegistered: false, decimals: token.decimals, symbol: token.symbol }
      }
