...
import { getTokenPairsDomain, getOrderBookDomain, getAccountBalancesDomain, getAccountDomain, getTokenDomain, getOrdersDomain, } from '../domains/'
...
export default function getOrderFormSelector(state: State) {
    ...
    let { baseTokenSymbol, quoteTokenSymbol, makeFee, takeFee, baseTokenDecimals, quoteTokenDecimals } = currentPair
...
export const sendNewOrder = (side: string, amount: number, price: number): ThunkAction => {
    return async (dispatch, getState, { socket, mixpanel }) => {
        ...
        try {
            ...
            let { baseTokenSymbol, quoteTokenSymbol, baseTokenDecimals, quoteTokenDecimals, makeFee, takeFee, } = pair
            ...
            order.side === 'BUY' ? sellTokenSymbol = quoteTokenSymbol : sellTokenSymbol = baseTokenSymbol
            ...
            order.side === 'BUY' ? totalSellAmount = quoteAmount.add(fee) : totalSellAmount = baseAmount
    ...
    return async (dispatch, getState, { txProvider, mixpanel }) => {
        ...
        try {
            ...
            const txConfirmHandler = (txConfirmed, txHash1, txHash2) => {
                ...
                txConfirmed ? dispatch(actionCreators.confirmUnlockPair(baseTokenSymbol, quoteTokenSymbol, tx1, tx2)) : dispatch(actionCreators.errorUnlockPair(baseTokenSymbol, quoteTokenSymbol, tx1, tx2, errorMessage))
            }
