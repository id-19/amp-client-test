...
const KeyIcon = props => {
  ...
  const trackIconClick = () => {
    mixpanel.track("Login Icon Clicked", {
      "Context": "Login Button",
      "Color": props.color, // Dynamic value based on props
      "Size": `${props.size}px`, // Dynamic value based on props
    });
  };

  return (
    <svg style={styles.svg} width={`${props.size}px`} height={`${props.size}px`} viewBox="0 0 71 75" version="1.1" xmlns="http://www.w3.org/2000/svg" id="key-icon" onClick={trackIconClick} // Added onClick event handler for tracking
    >
      <defs />
