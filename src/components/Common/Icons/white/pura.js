...
import { useMixpanel } from 'mixpanel-react'; // Assuming a custom hook for Mixpanel integration

const SvgPura = props => {
  const mixpanel = useMixpanel(); // Custom hook to access Mixpanel

  const trackIconInteraction = () => {
    mixpanel.track('Icon Interaction', {
      'Icon Name': 'SvgPura',
      'Action': 'Click',
      'Context': 'Assumed to be wrapped in a clickable element elsewhere in the application'
    });
  };

  return (
    <div onClick={trackIconInteraction}> {/* Wrapper to capture click events */}
