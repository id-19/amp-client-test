import React, { useEffect } from 'react';
import mixpanel from 'mixpanel-browser';
mixpanel.init("YOUR_PROJECT_TOKEN"); // Replace YOUR_PROJECT_TOKEN with your actual Mixpanel project token.

const SvgPlr = props => {
  useEffect(() => {
    // Track Icon Viewed event
    mixpanel.track("Icon Viewed", {
      "Context": "Rendered on the page",
      "Icon Name": "SvgPlr"
    });
  }, []); // Empty dependency array means this effect runs once on mount

  const handleClick = () => {
    // Track Icon Clicked event
    mixpanel.track("Icon Clicked", {
      "Context": "Assuming part of a button or clickable area",
      "Icon Name": "SvgPlr",
      "Action": "Clicked"
    });
  };

  return (
    <svg width={props.width || 64} height={props.height || 64} {...props} onClick={handleClick} // Added onClick event handler for tracking
         fill="#fff" fillRule="evenodd" >
      <path d="M16 32C7.163 32 0 24.837 0 16S7.163 0 16 0s16 7.163 16 16-7.163 16-16 16zM8.41 20.902c.496.507 1.246.738 2.257.735 2.16 0 3.954-1.866 3.954-4.997v-.39c0-3.01-1.55-5.262-3.914-5.262-.564 0-1.57.021-2.358.777H8.26v-.64H6V25h2.312v-4.098zm4.064-4.076c0 2.019-.865 2.933-2.038 2.936-1.49 0-2.249-1.274-2.249-2.939v-.68c0-1.747.771-3.271 2.209-3.271 1.331 0 2.078 1.31 2.078 3.271zM15.74 7v14.576h2.315V7zm8.929 6.409h.29l-.013-2.333c-.1 0-.015-.003-.119-.003-1.042-.015-2.31.241-2.827.787h-.088v-.637h-2.133v10.362h2.303v-5.844c0-1.302 1.289-2.332 2.587-2.332zm-.972 5.686v2.481H26v-2.481z" />
    </svg>
  );
};

export default SvgPlr;