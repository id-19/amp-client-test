...
const SvgGbp = props => {
  // Track the "Rendered" action once upon component mount
  useEffect(() => {
    mixpanel.track("SVG Icon Interaction", {
      "Icon": "GBP",
      "Action": "Rendered",
      "Size": props.width || 64 // Assuming width and height are always equal for simplicity
    });
  }, [props.width, props.height]); // Re-run if the size changes

  // Handler for tracking the "Clicked" action
  const handleClick = () => {
    mixpanel.track("SVG Icon Interaction", {
      "Icon": "GBP",
      "Action": "Clicked",
  ...
  return (
    <svg width={props.width || 64} height={props.height || 64} {...props} onClick={handleClick}>
      <defs>
        ...
        <path id="gbp_svg__e" d="M11.087 13.815v-2.332c0-3.676 2.219-5.983 6.075-5.983 2.932 0 4.57 1.242 5.838 2.84l-2.483 1.9c-.951-1.165-1.85-1.85-3.328-1.85-1.77 0-2.827 1.217-2.827 3.17v2.255h6.578v2.637h-6.578v4.335h8.585V23.5H9v-1.977l2.087-.609v-4.462H9v-2.637h2.087z" />
      ...
      <g fill="none" fillRule="evenodd">
        ...
        <use fill="url(#gbp_svg__c)" style={{ mixBlendMode: 'soft-light', }} xlinkHref="#gbp_svg__b" />
