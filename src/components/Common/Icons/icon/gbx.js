...
const SvgGbx = props => {
  useEffect(() => {
    if (window.Mixpanel) {
      window.Mixpanel.track("SVG Rendered", {
        "Icon Name": "Gbx",
        "Usage Context": "Please specify context"
      });
    }
  }, []);
  ...
  const handleClick = () => {
    if (window.Mixpanel) {
      window.Mixpanel.track("SVG Clicked", {
        "Icon Name": "Gbx",
        "Click Context": "Please specify context",
        "Location": "Please specify location if applicable"
      });
    }
  };
  ...
  return (
    <svg width={props.width || 64} height={props.height || 64} {...props} onClick={handleClick}>
      ...
      <g fill="none" fillRule="evenodd">
        ...
        <use fill="url(#gbx_svg__c)" style={{ mixBlendMode: 'soft-light', }} xlinkHref="#gbx_svg__b" />
