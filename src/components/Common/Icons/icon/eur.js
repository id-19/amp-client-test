...
const SvgEur = props => {
  const handleCurrencySelection = () => {
    mixpanel.track("Currency Selection", { "Currency": "EUR" });
  };

  const handleViewProductsClick = (e) => {
    e.preventDefault(); // Prevent default link behavior
    mixpanel.track("View Products by Currency", { "Currency": "EUR" });
    window.location.href = "/products-eur"; // Navigate programmatically
  };

  return (
    <div>
      <button type="button" aria-label="Select Euro Currency" className="currency-selection-button" onClick={handleCurrencySelection}>
        Select Currency
      </button>
      <a href="/products-eur" aria-label="View products in Euro" className="view-euro-priced-products-link" onClick={handleViewProductsClick}>
        View Products
      </a>
      <svg width={props.width || 64} height={props.height || 64} {...props}>
        ...
        <g fill="none" fillRule="evenodd">
          ...
          <use fill="url(#eur_svg__c)" style={{ mixBlendMode: 'soft-light', }} xlinkHref="#eur_svg__b" />
