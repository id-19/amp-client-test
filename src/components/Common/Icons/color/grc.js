...
const SvgGrc = props => {
  useEffect(() => {
    // Track Icon View event
    mixpanel.track("Icon View", {
      "Context": "GRC icon rendered and viewed by the user",
      "Location": "Common Icons",
      "Time": new Date().toISOString() // Assuming Time wants the current timestamp of view
    });
  }, []);
  ...
  const handleClick = () => {
    ...
    mixpanel.track("Icon Click", {
      ...
      "Location": "Common Icons",
      "Time": new Date().toISOString() // Assuming Time wants the current timestamp of click
    });
  };

  return (
    <svg width={props.width || 64} height={props.height || 64} fill="none" fillRule="evenodd" onClick={handleClick} {...props}>
