...
const SvgPolis = props => {
  const handleShareViaPolisClick = () => {
    mixpanel.track("Share Via Polis", {
      "Content ID": "ID of the content being shared",
      "User ID": "ID of the user sharing the content",
  ...
  return (
    <svg width={props.width || 64} height={props.height || 64} {...props}>
      <g fill="none">
        ...
        {/* Implementing the button with event tracking */}
        <foreignObject x="0" y="0" width="100%" height="100%">
          <button type="button" aria-label="Share via Polis" className="share-via-polis-button" onClick={handleShareViaPolisClick} style={{position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)'}}>
            Share
          </button>
        </foreignObject>
      </g>
